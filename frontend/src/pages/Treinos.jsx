...
export default function Treinos() {
  ...
  useEffect(() => {
    if (formData.tipo_treino === 'Personalizado' && formData.data && formData.hora_inicio && formData.hora_fim) {
      fetchInstrutoresDisponiveis();
    }
  }, [formData.data, formData.hora_inicio, formData.hora_fim, formData.tipo_treino]);
  ...
  const fetchInstrutoresDisponiveis = async () => {
    setLoadingInstrutores(true);
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`${API}/instrutores-disponiveis`, {
        params: {
          data: formData.data,
      ...
      setInstrutoresDisponiveis(response.data);
      
      if (response.data.length === 0) {
        toast.warning('Nenhum instrutor disponível neste horário');
      }
    } catch (error) {
      toast.error('Erro ao buscar instrutores disponíveis');
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.tipo_treino === 'Personalizado') {
      if (!formData.data || !formData.hora_inicio || !formData.hora_fim || !formData.instrutor_id_instrutor) {
        toast.error('Preencha todos os campos obrigatórios para treino personalizado');
        return;
      }
      
      payload.data = formData.data;
      payload.hora_inicio = formData.hora_inicio;
      payload.hora_fim = formData.hora_fim;
      payload.instrutor_id_instrutor = formData.instrutor_id_instrutor;
            ...
            <form onSubmit={handleSubmit} className="space-y-4" data-testid="treino-form">
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) => {
                    setFormData({ ...formData, tipo_treino: value });
                    setInstrutoresDisponiveis([]);
                  }}
                  required
                >
              ...
              {formData.tipo_treino === 'Personalizado' && (
                <>
                  <div className="border-t pt-4">
                    <h3 className="text-sm font-semibold text-gray-900 mb-3">Agendamento</h3>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <Input
                            ...
                            required
                            data-testid="treino-hora-inicio-input"
                          />
                        </div>
                        <div>
                          ...
                          <Input
                            ...
                            required
                            data-testid="treino-hora-fim-input"
                          />
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        {loadingInstrutores ? (
                          ...
                          <div className="text-sm text-red-600 bg-red-50 p-3 rounded-lg">
                            Nenhum instrutor disponível neste horário
                          </div>
                        ) : (
                          <div className="text-sm text-gray-500 bg-gray-50 p-3 rounded-lg">
                            Preencha data e horário para ver instrutores disponíveis
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="border-t pt-4">
                    <h3 className="text-sm font-semibold text-gray-900 mb-3">Detalhes do Treino</h3>
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <Textarea
                          ...
                          rows={3}
                          placeholder="Descreva o treino personalizado..."
              ...
              <Button
                type="submit"
                className="w-full"
                disabled={formData.tipo_treino === 'Personalizado' && loadingInstrutores}
                data-testid="treino-submit-button"
              >
            ...
            <table className="w-full" data-testid="treinos-table">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-sm font-semibold text-gray-900">Data/Horário</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {filteredTreinos.map((treino) => (
                  <tr key={treino.id_treino} className="hover:bg-gray-50" data-testid="treino-row">
                    <td className="px-6 py-4 text-sm text-gray-900 font-medium">{treino.nome_treino}</td>
                    ...
                    <td className="px-6 py-4 text-sm text-gray-600">{treino.instrutor_nome || '-'}</td>
                    <td className="px-6 py-4 text-sm text-gray-600">
                      {treino.data && treino.hora_inicio ? (
                        <>
                          {new Date(treino.data).toLocaleDateString('pt-BR')}<br />
